...
class DecisionTransformer(nn.Module):
    def __init__(self, state_dim, embed_dim, context_length=10, n_layers=3, n_heads=1, dropout=0.1, num_classes=3):
        super().__init__()

        self.embed_rtg = nn.Linear(1, embed_dim)
        self.embed_state = nn.Linear(state_dim, embed_dim)
        self.embed_action = nn.Embedding(num_classes, embed_dim)
    ...
    def forward(self, rtgs, states, actions, timesteps):
        ...
        input_embeds = torch.cat(
            [rtg_embedding.unsqueeze(2), state_embedding.unsqueeze(2), action_embedding.unsqueeze(2)],
            dim=2
        ...
        action_hidden = hidden_states[:, 2::3, :]  # Predict on action positions
